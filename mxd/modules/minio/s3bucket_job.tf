    ...
    template {
      ...
      spec {
        container {
          ...
          image   = "minio/mc"
          command = ["/bin/sh", "-c"]
          args    = ["mc config alias set minio http://${local.minio-url} ${var.minio-username} ${var.minio-password} && mc cp /opt/config/${local.file-name} minio/${local.bucket-name}/${var.humanReadableName}-${local.file-name}"]
          volume_mount {
